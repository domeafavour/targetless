...
async function listEvents(params?: ListEventsParams): Promise<EventWithCurrentRecord[]> {
...
async function getEventsStats(): Promise<EventsStats> {
  const snapshot = await readState();
...
async function getEvent(eventId: string): Promise<EventDetail> {
  const snapshot = await readState();
...
async function createEvent(input: CreateEventInput): Promise<EventWithCurrentRecord> {
  const title = normalizeTitle(input.title);
  if (!title) {
    throw new Error("Title is required");
  }

  const parsedCount = parseNonNegativeCount(input.count);
...
async function completeEvent(input: CompleteEventInput): Promise<EventWithCurrentRecord> {
  const event = await runTransaction([DB_EVENTS_STORE, DB_RECORDS_STORE], "readwrite", async (stores) => {
    const eventsStore = stores[DB_EVENTS_STORE];

    const existingEvent =
      (await requestToPromise<EventEntity | undefined>(eventsStore.get(input.eventId))) ?? null;
    if (!existingEvent) {
      throw new Error("Event not found");
    }
...
async function createRecord(input: CreateRecordInput): Promise<EventWithCurrentRecord> {
  const { event, record } = await runTransaction([DB_EVENTS_STORE, DB_RECORDS_STORE], "readwrite", async (stores) => {
    const eventsStore = stores[DB_EVENTS_STORE];
    ...
    const parsedCount = parseNonNegativeCount(input.count);
    const parsedNote = normalizeOptionalNote(input.note);
...
async function completeRecord(input: CompleteRecordInput): Promise<EventWithCurrentRecord> {
  const { event, currentRecord } = await runTransaction([DB_EVENTS_STORE, DB_RECORDS_STORE], "readwrite", async (stores) => {
    const eventsStore = stores[DB_EVENTS_STORE];
    const recordsStore = stores[DB_RECORDS_STORE];
    ...
    const existingRecord =
      (await requestToPromise<EventRecord | undefined>(recordsStore.get(existingEvent.currentRecordId))) ?? null;
    if (!existingRecord) {
...
async function updateEventTitle(input: UpdateEventTitleInput): Promise<EventWithCurrentRecord> {
  const title = normalizeTitle(input.title);
  if (!title) {
    throw new Error("Title is required");
  }
...
function attachCurrentRecord(event: EventEntity, records: EventRecord[]): EventWithCurrentRecord {
...
async function readPersistedCollections(): Promise<State | null> {
  ...
  if (!db.objectStoreNames.contains(DB_EVENTS_STORE) || !db.objectStoreNames.contains(DB_RECORDS_STORE)) {
    return null;
  }

  return new Promise((resolve, reject) => {
    const transaction = db.transaction([DB_EVENTS_STORE, DB_RECORDS_STORE], "readonly");
    ...
    eventsRequest.onerror = () => reject(eventsRequest.error ?? new Error("Failed to read events from IndexedDB"));
    ...
    recordsRequest.onerror = () => reject(recordsRequest.error ?? new Error("Failed to read records from IndexedDB"));

    transaction.onerror = () => reject(transaction.error ?? new Error("Failed to read state from IndexedDB"));
  });
}
...
function deleteRecordsByEvent(recordsStore: IDBObjectStore, eventId: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    cursorRequest.onerror = () => reject(cursorRequest.error ?? new Error("Failed to delete records"));
  });
}
